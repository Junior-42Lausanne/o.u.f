      ...
      customMask.graphics._activeInstructions.map((elt, idx) => {
        ...
        handle.on("pressmove", (e) => {
          ...
          if (handle.idx === instructionLength - 1) {
            customMask.graphics._activeInstructions[0].x =
              e.stageX - customMask.x + xOffset;
            customMask.graphics._activeInstructions[0].y =
              e.stageY - customMask.y;
          }
          customMask.graphics._activeInstructions[idx].x =
            e.stageX - customMask.x + xOffset;
          customMask.graphics._activeInstructions[idx].y =
            e.stageY - customMask.y;
        });
