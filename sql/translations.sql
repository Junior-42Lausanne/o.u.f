...
USING (
		project_id = NULL OR public.is_member_of_project(translations.project_id) OR public.is_superadmin()
);

CREATE POLICY "Insert translations" ON public.translations FOR INSERT
WITH CHECK (
		(project_id = NULL AND public.is_superadmin()) OR public.is_member_of_project_and_admin(translations.project_id)
);
